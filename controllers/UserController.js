...
const register = async (req, res) => {
    try {
        const { name, email, password, role, barbershopName, openingTime, closingTime, lunchBreak } = req.body;

        const hashedPassword = await bcrypt.hash(password, 10);

        if (role === 'gerente' && req.user?.role !== 'admin') {
            return res.status(403).json({ message: 'Apenas admin pode criar gerentes.' });
        }
        ...
        if (role === 'gerente') {
            ...
            if (existingBarbershop) return res.status(400).json({ message: 'Usuário ja possui uma barbearia.' });
