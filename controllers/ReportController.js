...
const getReportByPeriod = async (req, res) => {
	try {
		...
		const appointmentRevenues = transactions
			.filter(
				(transaction) =>
					transaction.entryType === "agendamento" &&
					transaction.status === "pago"
			)
			.reduce((acc, transaction) => acc + transaction.amount, 0);
		const productRevenues = transactions
			.filter(
				(transaction) => transaction.entryType === "produto-vendido"
			)
			.reduce((acc, transaction) => acc + transaction.amount, 0);
		const productsSold = transactions
			.filter(
				(transaction) => transaction.entryType === "produto-vendido"
			)
		...
		const totalServices = completedAppointments.reduce(
			(sum, appt) => sum + appt.services.length,
			0
		);
		const averageTicketAppointment =
			completedAppointments.length > 0
				? appointmentRevenues / completedAppointments.length
				: 0;

		const averageTicketService =
			totalServices > 0 ? appointmentRevenues / totalServices : 0;
