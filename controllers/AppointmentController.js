...
const createAppointment = async (req, res) => {
    try {
        const { clientName, barbershop, barber, services, date, hour, paymentMethod } = req.body;

        // Find barbershop by id
        const barbershopDoc = await Barbershop.findById(barbershop);
        if (!barbershopDoc) return res.status(404).json({ message: 'Barbearia nao encontrada.' });
        ...
        // // Get available slots
        const availableTimes = await availableSlotsService({ date, barber, barbershop });
        ...
        if (!isValidSlot(hour, availableTimes)) {
            return res.status(400).json({ message: "Horário inválido. Escolha um horário disponível." });
        }
...
const updateAppointment = async (req, res) => {
    try {
        const { id } = req.params;
        const { clientName, barbershop, barber, services, date, hour, status, paymentMethod } = req.body;
