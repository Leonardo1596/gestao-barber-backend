...
const createTransaction = async (req, res) => {
    try {
        const { type, entryType, barbershop, description, date, appointment, product, quantity, barber } = req.body;
        let amount = req.body.amount;

        if (entryType === 'produto-vendido') {
            const productDoc = await Product.findById(product);
            if (!productDoc) return res.status(404).json({ message: 'Produto nao encontrado.' });
            if (productDoc.quantity < quantity) return res.status(400).json({ message: 'Quantidade de produtos insuficiente.' });
...
const deleteTransaction = async (req, res) => {
    try {
        ...
        if (!transaction) return res.status(404).json({ message: 'Transação nao encontrada.' });
...
const getTransactionByPeriod = async (req, res) => {
    try {
        ...
        const transactions = await Transction.find({ barbershop, date: { $gte: startDate, $lte: endDate } });
        res.status(200).json(transactions);
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: 'Erro ao buscar transações' });
